...
def timed_cache(seconds=CACHE_DURATION, maxsize=128):
    def wrapper_decorator(func):
        ...
        def wrapper(*args, **kwargs):
            result, _ = time_bounded(
                *args, _cache_time=int(time.time() / seconds), **kwargs
            )
            return result
...
try:
    ...
    HAS_TABULATE = False
    logging.warning(
        "Biblioteca 'tabulate' não encontrada. A saída em tabela não estará disponível."
    )
...
RISK_PROFILES = {"low": 0.8, "medium": 1.0, "high": 1.5, "ultra": 2.0}
...
if POLYGON_API_KEY == "YOUR_API_KEY_HERE":
    logger.info(
        "Chave da Polygon não fornecida; prosseguindo sem Polygon para dados de mercado."
    )
...
try:
    ...
    logger.warning(
        "Biblioteca requests_cache não instalada. Recomenda-se instalar para melhor performance."
    )
...
class DataLoader:
    ...
    def get_asset_data(
        ...
        try:
            ...
            df = DataLoader._get_asset_data_yfinance(
                ticker, days, interval, include_prepost=extended_hours
            )
            ...
            if (
                ...
                and use_polygon
                and POLYGON_API_KEY != "YOUR_API_KEY_HERE"
            ):
                logger.info(
                    f"Data not found via yfinance for {ticker}, trying Polygon..."
                )
    ...
    def is_market_hours() -> tuple:
        ...
        try:
            ...
            is_regular_hours = is_weekday and market_open <= current_time < market_close

            # Check if we're in extended hours (pre or post)
            is_extended_hours = is_weekday and (
                (pre_market_open <= current_time < market_open)
                or (market_close <= current_time < post_market_close)  # Pre-market
            )  # Post-market
    ...
    @staticmethod
    def _get_asset_data_yfinance(
        ticker: str, days: int, interval: str = "1d", include_prepost: bool = False
    ) -> pd.DataFrame:
        ...
        try:
            ...
            try:
                ...
                if should_include_prepost:
                    logger.info(
                        f"Extended hours mode active for {ticker} (current market: {'regular' if is_regular_hours else 'extended' if is_extended_hours else 'closed'})"
                    )
            except Exception as e:
                # If market hours detection fails, fall back to the user's preference
                logger.warning(
                    f"Market hours detection failed: {e}. Using explicit include_prepost={include_prepost}"
                )
            ...
            try:
                ...
                if "include_prepost" in str(e) or "unexpected keyword argument" in str(
                    e
                ):
                    logger.warning(
                        f"include_prepost parameter not supported in this version of yfinance for {ticker}, falling back to default behavior"
                    )
                    df = stock.history(
    ...
    @staticmethod
    def _get_asset_data_polygon(
        ticker: str, days: int, interval: str = "1d"
    ) -> pd.DataFrame:
    ...
    def get_realtime_prices_bulk(tickers: list) -> dict:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            batch_results = list(
                executor.map(DataLoader._get_batch_prices, ticker_batches)
            )
        ...
        if missing:
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                futures = {
                    executor.submit(DataLoader._get_single_ticker_price, t): t
                    for t in missing
                }
...
class TechnicalIndicators:
    ...
    def ADX(df: pd.DataFrame, period: int = 14) -> pd.Series:
        ...
        dfc["TR"] = np.maximum(
            ...
            np.maximum(
                np.abs(
                    dfc["High"] - dfc["Close"].shift(1, fill_value=dfc["High"].iloc[0])
                ),
                np.abs(
                    dfc["Low"] - dfc["Close"].shift(1, fill_value=dfc["Low"].iloc[0])
                ),
            ),
        )
    ...
    def stochastic_oscillator(df: pd.DataFrame, period: int = 14):
        ...
        if (
            df.empty
            or period <= 0
            or not set(["High", "Low", "Close"]).issubset(df.columns)
        ):
        ...
        percent_k[non_zero_range] = 100 * (
            (df["Close"][non_zero_range] - lowest_low[non_zero_range])
            / range_diff[non_zero_range]
        )
    ...
    def OBV(df: pd.DataFrame) -> pd.Series:
        ...
        for i in range(1, len(df)):
            if (
                ...
                or pd.isna(df["Close"].iloc[i - 1])
                or pd.isna(df["Volume"].iloc[i])
            ):
    ...
    def add_all_indicators(df: pd.DataFrame, params: dict) -> pd.DataFrame:
        ...
        if df.empty or "Close" not in df.columns:
            logger.warning(
                "DataFrame vazio ou sem coluna 'Close' ao adicionar indicadores"
            )
            return df

        df_ind = df.copy()
        try:
            df_ind["SMA"] = TechnicalIndicators.SMA(
                df_ind["Close"], params.get("sma_period", 20)
            )
            df_ind["EMA"] = TechnicalIndicators.EMA(
                df_ind["Close"], params.get("ema_period", 9)
            )
            df_ind["RSI"] = TechnicalIndicators.RSI(
                df_ind["Close"], params.get("rsi_period", 14)
            )
            ...
            bb_mid, bb_upper, bb_lower = TechnicalIndicators.BollingerBands(
                df_ind["Close"], params.get("bb_window", 20), params.get("bb_std", 2)
            )
            ...
            if set(["High", "Low"]).issubset(df_ind.columns):
                ...
                df_ind["%K"], df_ind["%D"] = TechnicalIndicators.stochastic_oscillator(
                    df_ind
                )
                (
                    df_ind["Ichimoku_conv"],
                    df_ind["Ichimoku_base"],
                    df_ind["Ichimoku_spanA"],
                    df_ind["Ichimoku_spanB"],
                ) = TechnicalIndicators.ichimoku_cloud(df_ind)
...
class MarketAnalysis:
    ...
    def get_market_data() -> dict:
        ...
        for name, ticker in MARKET_REFERENCE_TICKERS.items():
            try:
                df = DataLoader.get_asset_data(
                    ticker, days=20, interval="1d", use_polygon=False
                )

                if df.empty or len(df) < 5:
                    logger.warning(
                        f"Dados insuficientes para análise de mercado: {ticker}"
                    )
                    continue

                last_close = df["Close"].iloc[-1]
                returns_1d = (
                    (last_close / df["Close"].iloc[-2] - 1) * 100
                    if len(df) >= 2
                    else 0.0
                )
                returns_5d = (
                    (last_close / df["Close"].iloc[-5] - 1) * 100
                    if len(df) >= 5
                    else 0.0
                )
...
class APIClient:
    @staticmethod
    def get_polygon_technical_data(
        ticker: str, indicator: str, params: dict = None
    ) -> dict:
    ...
    def get_polygon_related(ticker: str) -> list:
        ...
        try:
            ...
            logger.error(
                f"Error getting related tickers from Polygon for {ticker}: {e}"
            )
            return []
...
class QualitativeAnalysis:
    ...
    def qualitative_score(ticker: str) -> dict:
        try:
            ...
            sentiment = QualitativeAnalysis.analyze_news_sentiment(ticker)
            ...
            return {
                ...
                * 100,  # Convert to score scale (-100 to 100)
                "details": sentiment,
            }
        except Exception as e:
            logger.error(f"Error in qualitative analysis for {ticker}: {e}")
            return {"qualitative_score": 0, "details": {"error": str(e)}}
...
class FundamentalAnalysis:
    ...
    def fundamental_score(ticker: str) -> dict:
        ...
        try:
            ...
            details["actions"] = (
                actions.to_dict() if hasattr(actions, "to_dict") else {}
            )
            details["capital_gains"] = (
                capital_gains.to_dict() if hasattr(capital_gains, "to_dict") else {}
            )
            ...
            if quote_type in ["ETF", "MUTUALFUND"]:
                ...
                details[
                    "note"
                ] = f"Tipo de ativo não suportado para análise fundamental: {quote_type}"

            details["raw_score"] = score
            return {
                "fundamental_score": float(np.clip(score, -100, 100)),
                "details": details,
            }
...
class Strategy:
    @staticmethod
    def technical_score(
        df: pd.DataFrame, risk_profile: str = "medium", params: dict = DYNAMIC_PARAMS
    ) -> dict:
        """Calcula o score técnico com base nos indicadores do DataFrame."""
        if df.empty or "Close" not in df.columns or len(df) < 2:
            return {
                "score": 0.0,
                "details": {"error": "Dados insuficientes para análise técnica"},
            }
        ...
        # MACD
        macd_hist = (
            last.get("MACD_hist", 0) if not pd.isna(last.get("MACD_hist", 0)) else 0
        )
        ...
        if len(df) >= 6:
            ...
            if abs(recent_return) > 5:
                ...
                trend_score = (
                    10
                    if (recent_return > 0 and rsi_val < 60)
                    else (
                        -10
                        if recent_return < 0 and rsi_val > 40
                        else 5
                        if recent_return > 0
                        else -5
                    )
    ...
    def market_adaptive_score(ticker: str, market_sentiment: dict) -> float:
        ...
        try:
            ...
            is_inverse = (quote_type == "ETF") and any(
                term in info.get("shortName", "").upper()
                for term in ["SHORT", "INVERSE", "BEAR"]
            )
    ...
    def decision_engine(
        ...
        if goals and remaining_days is not None:
            recovery_urgency = Strategy.calculate_recovery_urgency(
        ...
        buy_partial_threshold = params["decision_buy_partial_threshold"] * (
            1 - adj_factor * gap_ratio
        )
        sell_threshold = params["decision_sell_threshold"] * (
            1 + adj_factor * gap_ratio
        )
        sell_partial_threshold = params["decision_sell_partial_threshold"] * (
            1 + adj_factor * gap_ratio
        )
        ...
        position_profit_pct = (
            ((current_price - avg_price) / avg_price * 100) if position_exists else 0.0
        )
        position_profit_value = (
            (current_price - avg_price) * quantity if position_exists else 0.0
        )
        ...
        if trailing_data is not None and position_exists:
            result_ts = check_trailing_stop(
                ticker, position, current_price, trailing_data, params
            )
        ...
        if not trailing_triggered:
            # Verificar stop loss
            if position_exists and position_profit_pct <= params.get(
                "stop_loss_pct", -8.0
            ):
                decision = "VENDER"
            ...
            elif position_exists and position_profit_pct >= params.get(
                "take_profit_pct", 5.0
            ):
                if total_score < 0:
                    decision = "VENDER"
        ...
        if (
            decision.upper() in ["VENDER", "REALIZAR LUCRO PARCIAL", "REDUZIR"]
            and position_exists
        ):
            sell_qty = (
                quantity
                if decision.upper() == "VENDER"
                else max(1, int(quantity * 0.5))
            )
...
def check_trailing_stop(
    ticker: str, position: dict, current_price: float, trailing_data: dict, params: dict
) -> dict:
    ...
    try:
        ...
        if ticker not in trailing_data:
            trailing_data[ticker] = {
                ...
                * (1 - params.get("trailing_stop_pct", 3.0) / 100),
            }
        ...
        if current_price > highest_price:
            ...
            logger.info(
                f"Atualizando trailing stop de {ticker}: novo topo ${highest_price:.2f}, stop ajustado para ${new_stop:.2f}"
            )
        ...
        if current_price <= stop_level:
            ...
            result[
                "reason"
            ] = f"Trailing stop acionado em ${stop_level:.2f} (máx recente ${highest_price:.2f})"
...
class PositionSizing:
    ...
    def calculate_position_size(
        ...
        base_risk = {"low": 0.01, "medium": 0.02, "high": 0.04, "ultra": 0.08}.get(
            risk_profile, risk_per_trade
        )
        ...
        if daily_goal > 0 and daily_gap > 0:
            adjustment_factor = min(
                1 + daily_gap / daily_goal,
                params.get("position_size_adjustment_max", 2.0),
            )
        ...
        if price > 0:
            effective_var = max(
                var, 0.005
            )  # Garantir um mínimo de VaR para evitar posições excessivas
        ...
        allocation_pct = (
            (allocation_value / account_balance * 100) if account_balance > 0 else 0.0
        )
    ...
    @staticmethod
    def _adjust_for_balance_and_commission(
        shares: int, price: float, balance: float
    ) -> int:
...
def analyze_portfolio(
    ...
    logger.info(
        f"Sentimento de mercado atual: {market_sent['sentiment'].upper()} (viés {market_sent['market_bias_score']:.1f})"
    )
    ...
    for ticker, pos in portfolio.items():
        try:
            df = DataLoader.get_asset_data(
                ticker, days=60, extended_hours=extended_hours
            )
            ...
            current_value = quantity * (
                current_price if current_price else df["Close"].iloc[-1]
            )
    ...
    portfolio_summary = {
        ...
        "lucro_prejuízo_pct": ((total_current / total_invested - 1) * 100)
        if total_invested > 0
        else 0.0,
...
def analyze_watchlist(
    ...
    # Calcular meta diária de recuperação
    daily_goal = (
        goals.get("target_recovery", 0) / max(1, remaining_days) if goals else 0
    )
...
def generate_rebalance_plan(
    ...
    for ticker, details in portfolio_analysis.get("ativos", {}).items():
        ...
        if decision in ["VENDER", "REALIZAR LUCRO PARCIAL", "REDUZIR"] and qty > 0:
            ...
            if operation_value < params.get("min_transaction_value", 50.0):
                logger.info(
                    f"Ignorando venda de {ticker}: valor muito baixo (${operation_value:.2f})"
                )
                continue
    ...
    for ticker, details in portfolio_analysis.get("ativos", {}).items():
        ...
        if (
            score < -20
            or position_profit_pct < params.get("min_sell_performance", -3.0)
        ) and qty > 0:
    ...
    for buy in buy_candidates[:3]:  # Considerar os top 3 candidatos
        additional_capital_needed += buy["operation_value"] + calculate_xp_commission(
            buy["operation_value"]
        )
    ...
    for candidate in buy_candidates:
        ...
        max_affordable_qty = (
            int((available_capital * 0.95) / candidate["current_price"])
        ...
        if (
            len(plan["buy"]) >= params.get("max_portfolio_assets", 5) // 2
            or available_capital < 100
        ):
            break

    # 8. Calcular estatísticas do plano de rebalanceamento
    initial_capital = account_balance
    capital_from_sales = sum(
        item["capital_freed"] for item in plan["sell"] + plan["rebalance"]
    )
    ...
    try:
        logger.debug(
            f"Returning rebalance plan: {len(plan['sell'])} sells, {len(plan['rebalance'])} rebalances, {len(plan['buy'])} buys"
        )
        return plan
    except Exception as e:
...
def create_backtest(
    ...
    if not portfolio_history or len(portfolio_history) < 2:
        return {"error": "Dados insuficientes para backtest", "metrics": {}}

    try:
        # Extrair retornos diários do portfólio
        portfolio_values = [
            snapshot.get("total_value", 0) for snapshot in portfolio_history
        ]
        portfolio_returns = []
        ...
        information_ratio = (sum(active_returns) / len(active_returns)) / (
            pd.Series(active_returns).std() or 1
        )
        ...
        return {"error": f"Erro ao realizar backtest: {str(e)}", "metrics": {}}
...
def calculate_performance_metrics(returns: list, risk_free_rate: float = 0.03) -> dict:
    ...
    sharpe_ratio = (
        excess_returns.mean() / (returns_array.std() or 0.0001) * np.sqrt(252)
    )
    ...
    return {
        ...
        "calmar_ratio": abs(annual_return / (max_drawdown * 100))
        if max_drawdown > 0
        else 0,
    }
        ...
        if format_type == "json":
            ...
            portfolio_df = pd.DataFrame(
                [
                    ...
                    for ticker, details in results["ativos"].items()
...
class TradingAPI:
    ...
    def place_order(
        self, ticker: str, qty: int, side: str, order_type: str = "market"
    ) -> dict:
...
class NotificationSystem:
    ...
    def alert_trade_recommendation(
        self, ticker: str, action: str, quantity: int, price: float, reason: str
    ) -> bool:
    ...
    for ticker, position in portfolio.items():
        ...
        if not isinstance(avg_price, (int, float)) or avg_price <= 0:
            validation["errors"].append(
                f"Preço médio inválido para {ticker}: {avg_price}"
            )
            validation["status"] = "error"
        ...
        if "last_buy" in position:
            ...
            if not isinstance(last_buy, dict) or "price" not in last_buy:
                validation["warnings"].append(
                    f"Dados de última compra inválidos para {ticker}"
                )
        ...
        if "last_sell" in position:
            ...
            if not isinstance(last_sell, dict) or "price" not in last_sell:
                validation["warnings"].append(
                    f"Dados de última venda inválidos para {ticker}"
                )
...
def parse_command_line_args():
    ...
    parser = argparse.ArgumentParser(
        description="Sistema de Trading Swing Trade Agressivo"
    )
    ...
    parser.add_argument(
        "--json-file",
        type=str,
        help="Arquivo JSON com portfolio, watchlist, account_balance e goals",
    )
    parser.add_argument(
        "--risk",
        type=str,
        ...
        help="Perfil de risco (default: high)",
    )
    parser.add_argument(
        "--output-format",
        type=str,
    ...
    parser.add_argument(
        "--use-polygon",
        action="store_true",
        help="Usar Polygon.io para dados (se disponível)",
    )
    parser.add_argument(
        "--extended",
        action="store_true",
        help="Incluir dados de extended hours (pré e pós mercado)",
    )
    ...
    parser.add_argument(
        "--validate-tickers",
        action="store_true",
        help="Validar tickers antes de analisar (mais lento)",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Exibir informações de debug durante execução",
    )
    parser.add_argument(
        "--export-file", type=str, help="Exportar resultados para arquivo"
    )
    parser.add_argument(
        "--export-format",
        type=str,
    ...
    parser.add_argument(
        "--simulate",
        action="store_true",
        help="Simular execução de rebalanceamento sem realizar ordens",
    )
    parser.add_argument(
        "--diagnostics", action="store_true", help="Executar diagnósticos do sistema"
    )
    parser.add_argument(
        "--version", action="store_true", help="Exibir versão do sistema"
    )
    parser.add_argument(
        "--backtest", action="store_true", help="Executar backtest com dados históricos"
    )
    parser.add_argument(
        "--backtest-days", type=int, default=30, help="Período para backtest em dias"
    )
    parser.add_argument(
        "--optimize", action="store_true", help="Executar otimização de carteira"
    )
    parser.add_argument(
...
def interactive_mode():
    ...
    if goals:
        print(
            f"Meta de recuperação: ${goals.get('target_recovery', 0):,.2f} em {goals.get('days', 0)} dias"
        )
    ...
    while True:
        ...
        if choice == "1":
            risk_profile = (
                input("Perfil de risco (low, medium, high, ultra) [high]: ") or "high"
            )
            trailing_data = {}
            ...
            output_format = (
                input("Formato de saída (text, table, json) [text]: ") or "text"
            )
            ...
            rebalance_plan = generate_rebalance_plan(
                portfolio_analysis, watchlist_analysis, account_balance, DYNAMIC_PARAMS
            )
            ...
            if output_format == "json":
                ...
                output_table_results(
                    portfolio_analysis, watchlist_analysis, rebalance_plan
                )
            else:
                output_text_results(
                    portfolio_analysis, watchlist_analysis, rebalance_plan
            ...
            target_cash_reserve_pct = (
                float(input("Reserva de caixa % [10]: ") or "10") / 100
            )
            ...
            for asset in results["assets_to_hold"]:
                print(
                    f"- {asset['ticker']}: {asset['qty']} ações, ${asset['current_value']:,.2f}"
                )

            print("\nAtivos a vender:")
            for asset in results["assets_to_sell"]:
                print(
                    f"- {asset['ticker']}: {asset['sell_qty']} ações, ${asset['price']:,.2f}/ação"
                )

            print("\nAtivos a comprar:")
            for asset in results["assets_to_buy"]:
                print(
                    f"- {asset['ticker']}: {asset['buy_qty']} ações, ${asset['price']:,.2f}/ação"
                )
            ...
            if results["errors"]:
                ...
                for error in results["errors"]:
                    print(
                        f"- {error.get('ticker', 'Unknown')}: {error.get('error', 'Unknown error')}"
                    )

            # Salvar resultado?
            if (
                input("\nDeseja salvar o resultado da simulação? (s/n) [n]: ").lower()
                == "s"
            ):
            ...
            if "error" in backtest_results:
                ...
                print(
                    f"Retorno anualizado: {portfolio_metrics.get('annual_return_pct', 0):.2f}%"
                )
                print(
                    f"Volatilidade: {portfolio_metrics.get('volatility_pct', 0):.2f}%"
                )
                print(f"Sharpe Ratio: {portfolio_metrics.get('sharpe_ratio', 0):.2f}")
                print(
                    f"Máximo Drawdown: {portfolio_metrics.get('max_drawdown_pct', 0):.2f}%"
                )
                ...
                print(
                    f"Information Ratio: {backtest_results.get('information_ratio', 0):.2f}"
                )
                print(
                    f"Retorno ativo: {backtest_results.get('active_return', 0) * 100:.2f}%"
                )
                print(
                    f"Tracking Error: {backtest_results.get('tracking_error', 0) * 100:.2f}%"
                )
            ...
            for source, info in diagnostics["data_access"].items():
                if "error" in info:
                    ...
                    print(
                        f"- {source}: {'OK' if info.get('success', False) else 'Falha'}"
                    )
            ...
            # Exportar dados
            export_format = (
                input("Formato de exportação (json, csv) [json]: ") or "json"
            )
...
class RecoveryTracker:
    ...
    def __init__(
        self, goals: dict, initial_portfolio_value: float, initial_balance: float
    ):
        ...
        self.daily_snapshots.append(
            {
                ...
                - (
                    self.daily_snapshots[-1]["total_value"] - self.start_value
                    if self.daily_snapshots
                    else 0
                ),
                "cumulative_pnl": current_pnl,
                "progress_pct": (current_pnl / self.target_recovery * 100)
    ...
    def get_status(self) -> dict:
        ...
        expected_progress = (
            (days_passed / self.total_days) * self.target_recovery
        ...
        return {
            ...
            "projected_completion_pct": (projection / self.target_recovery * 100)
            if self.target_recovery > 0
            else 0,
        }
...
class PortfolioExecutor:
    ...
    def execute_rebalance_plan(self, rebalance_plan: dict) -> dict:
        ...
        for sell in rebalance_plan.get("sell", []):
            try:
                result = self._execute_sell(
                    sell["ticker"], sell["sell_quantity"], sell["current_price"]
                )
        ...
        for rebalance in rebalance_plan.get("rebalance", []):
            try:
                ...
                logger.error(
                    f"Erro ao executar rebalanceamento de {rebalance['ticker']}: {e}"
                )
                results["errors"].append(
                    {
            ...
            try:
                ...
                if total_cost > self.account_balance:
                    ...
                    adjusted_qty = self._calculate_max_affordable_quantity(
                        buy["current_price"], self.account_balance
                    )
                    ...
                    logger.warning(
                        f"Ajustando quantidade de compra para {buy['ticker']} de {buy['buy_quantity']} para {adjusted_qty} devido ao saldo disponível"
                    )
                ...
                result = self._execute_buy(
                    buy["ticker"], buy["buy_quantity"], buy["current_price"]
                )
        ...
        results["executed_transactions"] = (
            len(results["sells_executed"])
            + len(results["rebalances_executed"])
            + len(results["buys_executed"])
        )
    ...
    def _execute_sell(
        self, ticker: str, quantity: int, price: float, is_rebalance: bool = False
    ) -> dict:
    ...
    def _calculate_max_affordable_quantity(
        self, price: float, available_balance: float
    ) -> int:
    ...
    def _calculate_portfolio_summary(self) -> dict:
        ...
        return {
            ...
            "total_profit_loss_pct": (
                (total_current_value / total_invested * 100) - 100
            )
            if total_invested > 0
            else 0,
    ...
    def get_market_movers(category="gainers", count=10):
        ...
        try:
            ...
            if category == "gainers":
                ...
                logger.warning(
                    f"Unknown category: {category}. Using 'gainers' instead."
                )
                screener = screen.get_day_gainers()
    ...
    def update_watchlist_with_market_movers(
        data, categories=None, replace=False, count_per_category=5
    ):
    ...
    def refresh_watchlist(
        file_path, categories=None, replace=False, count_per_category=5
    ):
...
class PortfolioOptimizer:
    ...
    def __init__(
        ...
        self.market_sentiment = (
            market_sentiment or MarketAnalysis.get_market_sentiment()
        )
    ...
    def optimize(
        ...
        target_cash_reserve_pct: float = 0.1,
        max_position_size_pct: float = 0.2,
        min_score_threshold: float = 30,
    ) -> dict:
        ...
        # Obter preços atuais
        all_tickers = list(self.portfolio.keys()) + [
            t for t in self.watchlist.keys() if self.watchlist[t].get("monitor", False)
        ]
        ...
        for ticker, position in self.portfolio.items():
            try:
                ...
                qual_result = QualitativeAnalysis.qualitative_score(ticker)
                fund_result = FundamentalAnalysis.fundamental_score(ticker)
                market_score = Strategy.market_adaptive_score(
                    ticker, self.market_sentiment
                )
        ...
        for ticker, data in self.watchlist.items():
            ...
            try:
                ...
                logger.error(
                    f"Erro ao analisar watchlist {ticker} para otimização: {e}"
                )

        # 2. Construir carteira ideal
        # Calcular valor total do portfólio
        total_portfolio_value = (
            sum(pos["position_value"] for pos in portfolio_analysis.values())
            ...
            all_assets.append(
                {
                    ...
                    "current_allocation_pct": analysis["position_value"]
                    / total_portfolio_value
                    * 100
        ...
        for asset in top_assets:
            ...
            if asset["in_portfolio"]:
                if difference > 0:
                    # Comprar mais
                    qty_to_buy = (
                        int(difference / asset["price"]) if asset["price"] > 0 else 0
                    )
                    ...
                    # Vender parcialmente
                    qty_to_sell = (
                        int(abs(difference) / asset["price"])
                        if asset["price"] > 0
                        else 0
                    )
                ...
                qty_to_buy = (
                    int(target_value / asset["price"]) if asset["price"] > 0 else 0
                )
            ...
            target_portfolio.append(
                {
                    "ticker": asset["ticker"],
                    "target_value": target_value,
                    "target_allocation_pct": target_value / total_portfolio_value * 100
                    if total_portfolio_value > 0
                    else 0,
                    "current_value": asset["current_position_value"],
        ...
        for asset in all_assets:
            if asset["in_portfolio"] and not any(
                t["ticker"] == asset["ticker"] for t in top_assets
            ):
                assets_to_sell.append(
                    {
                        "ticker": asset["ticker"],
        ...
        self.optimization_results = {
            ...
            "total_current_invested": sum(
                pos["position_value"] for pos in portfolio_analysis.values()
            ),
            "total_target_invested": sum(
                asset["target_value"] for asset in target_portfolio
            ),
            ...
            "sell_value": sum(
                sell["sell_qty"] * sell["price"] for sell in assets_to_sell
            ),
            "max_positions": max_positions,
            "target_cash_reserve_pct": target_cash_reserve_pct,
    ...
    def generate_rebalance_plan(self) -> dict:
        ...
        capital_from_sales = sum(
            item["capital_freed"] for item in rebalance_plan["sell"]
        )
        ...
        rebalance_plan["stats"] = {
            ...
            "capital_usado_compras": capital_for_buys,
            "saldo_remanescente": self.account_balance
            + capital_from_sales
            - capital_for_buys,
            "total_comissoes": sum(s["commission"] for s in rebalance_plan["sell"])
            + sum(b["commission"] for b in rebalance_plan["buy"]),
...
def main():
    ...
    if not args.json_file:
        print(
            "Por favor, especifique o arquivo JSON de configuração usando --json-file"
        )
    ...
    if args.debug:
        print(
            f"Dados carregados: {len(portfolio)} ativos em carteira, {len(watchlist)} ativos em watchlist"
        )
        print(f"Saldo: ${account_balance:,.2f}")
        if goals:
            print(
    ...
    if args.validate_tickers:
        ...
        for ticker in watchlist.keys():
            if watchlist[ticker].get(
                "monitor", False
            ) and not DataLoader.check_ticker_valid(ticker):
    ...
    if args.optimize:
        ...
        if args.output_format == "json":
            ...
            print("\n===== RESULTADO DA OTIMIZAÇÃO =====")
            print(
                f"Valor total da carteira: ${optimization_results['total_portfolio_value']:,.2f}"
            )
            print(f"Saldo atual: ${optimization_results['current_cash']:,.2f}")
            print(
                f"Reserva de caixa alvo: ${optimization_results['target_cash_reserve']:,.2f}"
            )
        ...
        if args.export_file:
            if export_results(
                optimization_results, args.export_file, args.export_format
            ):
    ...
    if args.backtest:
        ...
        for ticker, position in portfolio.items():
            ...
            if not df.empty and "Close" in df.columns:
                current_values[ticker] = (
                    position.get("quantity", 0) * df["Close"].iloc[-1]
                )
        ...
        portfolio_history.append(
            {
                "date": (datetime.now() - timedelta(days=args.backtest_days)).strftime(
                    "%Y-%m-%d"
                ),
        ...
        # Executar backtest
        benchmark = (
            args.backtest_benchmark if hasattr(args, "backtest_benchmark") else "SPY"
        )
        backtest_results = create_backtest(
            portfolio_history, benchmark, args.backtest_days
        )
    ...
    print("Analisando watchlist...")
    watchlist_analysis = analyze_watchlist(
        watchlist, account_balance, args.risk, extended_hours=args.extended, goals=goals
    )
...
def output_text_results(portfolio_analysis, watchlist_analysis, rebalance_plan):
    ...
    print(
        f"Lucro/Prejuízo: ${resumo.get('lucro_prejuizo_valor', 0):,.2f} ({resumo.get('lucro_prejuizo_pct', 0):.2f}%)"
    )
    ...
    for ticker, details in portfolio_analysis.get("ativos", {}).items():
        ...
        print(
            f"  Lucro/Prejuízo: ${details.get('position_profit_value', 0):,.2f} ({details.get('position_profit_pct', 0):.2f}%)"
        )
        print(f"  Score: {details.get('total_score', 0):.2f}")
        print(f"  Decisão: {details.get('decision', 'AGUARDAR')}")
        print(f"  Justificativa: {details.get('justificativa', '')}")
    ...
    print(
        f"Capital liberado com vendas: ${stats.get('capital_liberado_vendas', 0):,.2f}"
    )
...
def output_table_results(portfolio_analysis, watchlist_analysis, rebalance_plan):
    ...
    resumo_table = [
        ...
        [
            "Lucro/Prejuízo",
            f"${resumo.get('lucro_prejuizo_valor', 0):,.2f} ({resumo.get('lucro_prejuizo_pct', 0):.2f}%)",
        ],
    ...
    stats_table = [
        ...
        [
            "Capital liberado com vendas",
            f"${stats.get('capital_liberado_vendas', 0):,.2f}",
        ],
        [
            "Capital total disponível",
            f"${stats.get('capital_total_disponivel', 0):,.2f}",
        ],
